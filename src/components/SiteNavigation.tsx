...
const AALCornerAccent = ({ position, color = "strata-orange" }: { position: 'tl' | 'tr' | 'bl' | 'br', color?: string }) => {
  ...
  const lineClasses = {
    tl: { h: 'top-0 left-0 w-5 h-px bg-gradient-to-r', v: 'top-0 left-0 w-px h-5 bg-gradient-to-b' },
    tr: { h: 'top-0 right-0 w-5 h-px bg-gradient-to-l', v: 'top-0 right-0 w-px h-5 bg-gradient-to-b' },
    bl: { h: 'bottom-0 left-0 w-5 h-px bg-gradient-to-r', v: 'bottom-0 left-0 w-px h-5 bg-gradient-to-t' },
    br: { h: 'bottom-0 right-0 w-5 h-px bg-gradient-to-l', v: 'bottom-0 right-0 w-px h-5 bg-gradient-to-t' },
  };
  ...
  return (
    <div className={`absolute ${positionClasses[position]} w-5 h-5 pointer-events-none`}>
      ...
      <motion.div 
        className={`absolute ${position.includes('t') ? 'top-0' : 'bottom-0'} ${position.includes('l') ? 'left-0' : 'right-0'} w-1.5 h-1.5 rotate-45 border border-${color}/50`}
...
const KumaGlassPanel = ({ children, className = "" }: { children: React.ReactNode, className?: string }) => (
  <div 
    className={`relative backdrop-blur-md border border-white/[0.08] ${className}`}
    style={{
  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <motion.button 
          className="fixed bottom-6 right-6 z-50 group focus:outline-none focus:ring-2 focus:ring-strata-orange focus:ring-offset-2 focus:ring-offset-strata-black"
      ...
      <SheetContent 
        side="right" 
        className="w-[420px] p-0 border-l border-strata-steel/20 overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none opacity-[0.03]">
          {[...Array(20)].map((_, i) => (
            <div 
              key={i}
              className="absolute top-0 bottom-0 w-px bg-gradient-to-b from-white via-white to-transparent"
              style={{ left: `${(i + 1) * 5}%` }}
            />
          ))}
        </div>
        ...
        <nav 
          id="main-navigation"
          className="p-6 space-y-6 overflow-y-auto max-h-[calc(100vh-180px)] scrollbar-thin scrollbar-thumb-strata-steel/30" 
        ...
        >
          ...
          <section aria-labelledby="nav-industry-heading">
            <h3 
              ...
              className="text-[9px] font-mono text-strata-silver/40 uppercase tracking-[0.3em] mb-4 px-1 flex items-center gap-2"
            >
              ...
              {industryItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <Link
                      to={item.path}
                      className={`group relative block p-4 rounded-lg transition-all duration-300 ${
                        isActive
                          ? "bg-strata-cyan/10 border border-strata-cyan/30"
                          : "bg-strata-steel/5 border border-transparent hover:bg-strata-steel/15 hover:border-strata-steel/20"
                      }`}
                      ...
                      <div 
                        className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 transition-all ${
                          isActive
                            ...
                            : "bg-strata-steel/20 text-strata-silver group-hover:text-strata-cyan group-hover:bg-strata-cyan/10"
                        }`}
                      >
            ...
            <Link
              ...
              className={`group relative flex items-center gap-3 p-3 rounded-lg transition-all duration-300 ${
                isActive
                  ...
                  : "hover:bg-strata-steel/10 border border-transparent hover:border-strata-steel/10"
              }`}
              aria-current={isActive ? "page" : undefined}
            >
              ...
              {isActive && (
                <motion.div 
                  className={`absolute left-0 top-2 bottom-2 w-0.5 rounded-full bg-${accentColor}`}
                  layoutId={`${title}-active-indicator`}
                />
              )}

              <div 
                className={`${compact ? 'w-8 h-8' : 'w-10 h-10'} rounded-lg flex items-center justify-center transition-all ${
                  isActive
                    ? `bg-${accentColor} text-strata-white`
                    : `bg-strata-steel/20 text-strata-silver group-hover:text-${accentColor} group-hover:bg-${accentColor}/10`
                }`}
              >
              ...
              {isActive ? (
                <Badge className={`bg-${accentColor}/20 text-${accentColor} text-[8px] border-0 font-mono`}>
                  HERE
                </Badge>
              ) : (
                <ChevronRight className="w-4 h-4 text-strata-silver/30 group-hover:text-strata-silver/60 group-hover:translate-x-0.5 transition-all" />
              )}
            </Link>
          </motion.li>
        );
...
export const SiteMapPage = () => {
  ...
  return (
    <main 
      id="main-content"
      className="min-h-screen bg-gradient-to-b from-strata-charcoal to-strata-black p-8"
      role="main"
    >
      {/* Kuma slat overlay */}
    ...
    <ul className="space-y-2">
      {items.map((item) => (
        <li key={item.path}>
          <Link
            to={item.path}
            className="group flex items-center gap-3 p-3 rounded-lg hover:bg-strata-steel/10 transition-all"
          >
