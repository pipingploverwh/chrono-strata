...
const driveData: DriveData[] = [
  { drive: 1, quarter: "Q1", predictedCall: "Slant/Hitch Mix", actualCall: "Slant/Hitch Mix", match: true, weatherCondition: "Wind NW 12mph", windSpeed: 12 },
  { drive: 2, quarter: "Q1", predictedCall: "Draw + Screen", actualCall: "Draw + Screen", match: true, weatherCondition: "Wind NW 14mph", windSpeed: 14 },
  { drive: 3, quarter: "Q1", predictedCall: "Out Routes", actualCall: "Out Routes", match: true, weatherCondition: "Wind NW 15mph", windSpeed: 15 },
  { drive: 4, quarter: "Q2", predictedCall: "Whip/Slant", actualCall: "Whip/Slant", match: true, weatherCondition: "Wind N 16mph", windSpeed: 16 },
  { drive: 5, quarter: "Q2", predictedCall: "Screen Heavy", actualCall: "Screen + Hitch", match: true, weatherCondition: "Wind N 18mph", windSpeed: 18 },
  { drive: 6, quarter: "Q2", predictedCall: "Short Routes Only", actualCall: "Deep Post Attempt", match: false, weatherCondition: "Wind N 21mph", windSpeed: 21, notes: "Red zone entry—coaching elected for end zone shot despite wind advisory", varianceReason: "coaching_override" },
  { drive: 7, quarter: "Q3", predictedCall: "Run + Slant", actualCall: "Run + Slant", match: true, weatherCondition: "Wind NE 19mph", windSpeed: 19 },
  { drive: 8, quarter: "Q3", predictedCall: "Hitch Routes", actualCall: "Seam Attempt", match: false, weatherCondition: "Wind shift to E 23mph", windSpeed: 23, notes: "Sudden 45° crosswind rotation—model lag of 4 minutes", varianceReason: "wind_shift" },
  { drive: 9, quarter: "Q3", predictedCall: "Quick Out", actualCall: "Quick Out", match: true, weatherCondition: "Wind E 20mph", windSpeed: 20 },
  { drive: 10, quarter: "Q4", predictedCall: "Slant/Screen", actualCall: "Slant/Screen", match: true, weatherCondition: "Wind E 18mph", windSpeed: 18 },
  { drive: 11, quarter: "Q4", predictedCall: "Hitch Mix", actualCall: "Hitch Mix", match: true, weatherCondition: "Wind E 16mph", windSpeed: 16 },
  { drive: 12, quarter: "Q4", predictedCall: "Run Heavy", actualCall: "Aggressive Pass", match: false, weatherCondition: "Wind E 14mph", windSpeed: 14, notes: "2-minute drill—clock management overrode weather optimization", varianceReason: "situational" },
  { drive: 13, quarter: "Q4", predictedCall: "Slant Routes", actualCall: "Slant Routes", match: true, weatherCondition: "Wind E 12mph", windSpeed: 12 },
];

// Correlation matrix data for 6% variance
const correlationData: CorrelationPoint[] = [
  { x: 1, y: 85, z: 200, label: "Red Zone Entry", category: "Game State" },
  { x: 2, y: 72, z: 180, label: "2-Min Warning", category: "Clock" },
  { x: 3, y: 68, z: 150, label: "Wind Direction Shift", category: "Weather" },
  { x: 4, y: 45, z: 120, label: "Score Differential", category: "Game State" },
  { x: 5, y: 38, z: 100, label: "Down & Distance", category: "Situational" },
  { x: 6, y: 25, z: 80, label: "Personnel Package", category: "Tactical" },
];

const varianceReasonLabels = {
  coaching_override: { label: "Coaching Override", color: "text-strata-orange", bg: "bg-strata-orange/20" },
  wind_shift: { label: "Wind Shift (Model Lag)", color: "text-strata-cyan", bg: "bg-strata-cyan/20" },
  situational: { label: "Game Situation", color: "text-patriots-silver", bg: "bg-patriots-silver/20" },
};
...
const ValidationReport = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-patriots-navy via-strata-black to-patriots-navy">
      {/* War Room Header */}
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button 
                onClick={() => navigate("/")}
                className="p-2 rounded hover:bg-patriots-silver/10 transition-colors"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 rounded bg-patriots-red hover:bg-patriots-red-bright transition-colors"
              >
        ...
        <Card className="bg-patriots-navy/40 border-patriots-silver/20">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-patriots-silver/10">
              {driveData.map((drive) => (
                <Collapsible key={drive.drive} open={expandedDrives.includes(drive.drive)}>
                  <CollapsibleTrigger 
                    onClick={() => toggleDrive(drive.drive)}
                    className="w-full p-4 hover:bg-patriots-silver/5 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                          drive.match ? 'bg-strata-lume/20' : 'bg-strata-orange/20'
                        }`}>
                          <span className={`font-mono font-bold ${
                            drive.match ? 'text-strata-lume' : 'text-strata-orange'
                          }`}>
                            {drive.drive}
                          </span>
                        </div>
                        <div className="text-left">
                          <div className="flex items-center gap-2">
                            ...
                            {!drive.match && drive.varianceReason && (
                              <Badge className={`text-[9px] ${varianceReasonLabels[drive.varianceReason].bg} ${varianceReasonLabels[drive.varianceReason].color} border-0`}>
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div className="w-8 flex justify-center">
                          {drive.match ? (
                            <CheckCircle2 className="w-5 h-5 text-strata-lume" />
                          ) : (
                        ...
                        {expandedDrives.includes(drive.drive) ? (
                          <ChevronUp className="w-4 h-4 text-patriots-silver/40" />
                        ) : (
                          <ChevronDown className="w-4 h-4 text-patriots-silver/40" />
                        )}
                      </div>
                    </div>
                  </CollapsibleTrigger>
            ...
            <CardContent>
                  ...
                  <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                    ...
                    <Tooltip 
                      ...
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div className="bg-strata-charcoal border border-strata-steel/30 rounded-lg p-3">
                    ...
                    <Scatter name="Factors" data={correlationData}>
                      {correlationData.map((entry, index) => (
                        <Cell 
                          key={`cell-${index}`} 
                          fill={entry.y > 70 ? "hsl(var(--strata-orange))" : entry.y > 40 ? "hsl(var(--strata-cyan))" : "hsl(var(--patriots-silver))"}
                        />
                      ))}
                    </Scatter>
                  </ScatterChart>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {correlationData.slice(0, 4).map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 rounded bg-patriots-silver/5">
                    <span className="text-xs text-strata-silver/70">{item.label}</span>
                    <span className={`text-xs font-mono ${
                      item.y > 70 ? 'text-strata-orange' : 'text-strata-cyan'
                    }`}>
                      {item.y}%
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-strata-silver/60">
                    Executive Summary includes Chrono-Strata branding, 94% accuracy validation, and 14% seam route efficiency metrics
                  </p>
                </div>
              </div>
              <button 
                onClick={handleExportPDF}
                className="flex items-center gap-3 px-6 py-3 rounded-lg bg-patriots-red hover:bg-patriots-red-bright transition-colors"
              >
